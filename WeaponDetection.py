...
main = tkinter.Tk()
main.title("Video Analysis for Weapon detection and Alerting")
main.geometry("1300x900")

global model, classes, layer_names, output_layers, colors, filename, dataset,X_train,Y_train
...
def uploadDataset():
    ...
    # target_names = [layer_names[i] for i in range(len(layer_names))]
...
def loadModel():
    ...
    model = cv2.dnn.readNet("model/weapon_training_2000.weights", "model/weapon_testing.cfg")
    classes = ['Weapon']
...
def graph():
    ...
    with open("model/accuracy.txt", "r") as f:
        accurac = float(f.read())
    print("Training Model Accuracy = ", accurac)
    ...
    # plt.text(0.80, 0.80, f"Accuracy: {accurac:.2f}", ha='left', va='top',weight='bold', transform=plt.gca().transAxes)
    plt.show()
...
font = ('times', 16, 'bold')
title = Label(main, text='Video Analysis for Weapon Detection and Alerting',anchor=W, justify=LEFT)
